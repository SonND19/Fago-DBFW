...
create_pgsql_db()
{
  ...
  if [ "$POSTGRES_LOCATION" = "local" ]; then
    if [ "$POSTGRES_AUTH" = "ident" ]; then
      if su - postgres -c "$PSQL -c \"select * from pg_database where datname='$GREENSQL_DB_NAME';\" | grep -q \"$GREENSQL_DB_NAME\""; then
        ...
        echo -n "Do you want to drop and recreate database1 $GREENSQL_DB_NAME [N/y]: "
        read cont
      ...
      if $PSQL -h 127.0.0.1 postgres $POSTGRES_ADMIN_USER -c "select * from pg_database where datname='$GREENSQL_DB_NAME';" | grep -q "$GREENSQL_DB_NAME"; then
        ...
        echo " "
        echo "database $GREENSQL_DB_NAME already exists."
        echo -n "Do you want to drop and recreate database2 $GREENSQL_DB_NAME [N/y]: "
        read cont
        if [ "$cont" = "Y" ] || [ "$cont" = "y" ]; then
          SKIP_DB=0
        else
  ...
  then
    ...
    then
        ...
        echo " "
        echo "database $GREENSQL_DB_NAME already exists."
        echo -n "Do you want to drop and recreate database3 $GREENSQL_DB_NAME [N/y]: "
        read cont
        if [ "$cont" = "Y" ] || [ "$cont" = "y" ]; then
          SKIP_DB=0
        else
...
create_mysql_db()
{
  ...
  if $MYSQL $MRO -BNe 'show databases' | grep -q "$GREENSQL_DB_NAME"; then
    ...
    echo " "
    echo "database $GREENSQL_DB_NAME already exists."
    echo -n "Do you want to drop and recreate database4 $GREENSQL_DB_NAME [N/y]: "
    read cont
    if [ "$cont" = "Y" ] || [ "$cont" = "y" ]; then
      SKIP_DB=0
      echo -n "$SKIP_DB"
...
create_user_questions()
{
  ...
  read cont
  if [ "$cont" != "" ]; then
    # GREENSQL_DB_PWD = "Dson@2001"
    GREENSQL_DB_PWD=$cont
    # GREENSQL_DB_PWD = "SON123"
    echo -n "$GREENSQL_DB_PWD"
  fi
}
...
create_user_mysql()
{
  ...
  then
    #IDENTIFIED BY '${GREENSQL_DB_PWD}'
    if ! $MYSQL $MRO $GREENSQL_DB_NAME -f -e "GRANT ALL ON $GREENSQL_DB_NAME.* TO '$GREENSQL_DB_USER'@'localhost' "; then
      # echo $MYSQL;
      # echo $MRO;
      # echo $GREENSQL_DB_NAME;
      # echo $GREENSQL_DB_USER;
      # echo $GREENSQL_DB_PWD;
      echo $MYSQL $MRO $GREENSQL_DB_NAME -f -e "GRANT ALL ON $GREENSQL_DB_NAME.* TO '$GREENSQL_DB_USER'@'localhost' IDENTIFIED BY '${GREENSQL_DB_PWD}'";
      echo "Failed to create user1 $GREENSQL_DB_USER"
      echo "Aborting.."
      exit 0
    fi
  else
   if ! $MYSQL $MRO $GREENSQL_DB_NAME -f -e "GRANT ALL ON $GREENSQL_DB_NAME.* TO '$GREENSQL_DB_USER'@'%' IDENTIFIED BY '${GREENSQL_DB_PWD}'"; then
      echo "Failed to create user2 $GREENSQL_DB_USER"
      echo "Aborting.."
      exit 0
   fi
  fi
    ...
    if [ "$POSTGRES_AUTH" = "ident" ]; then
      ...
      if [ "$SKIP_USER" = "0" ]; then
        ...
        if ! su - postgres -c "$PSQL -c \"CREATE USER $GREENSQL_DB_USER WITH PASSWORD '$GREENSQL_DB_PWD';\""; then
          echo "Failed to create user3 $GREENSQL_DB_USER"
          echo "Aborting.."
          exit 0
        fi
      fi
      ...
      if [ "$SKIP_USER" = "0" ]; then
        ...
        if ! $PSQL -h 127.0.0.1 postgres $POSTGRES_ADMIN_USER -c "CREATE USER $GREENSQL_DB_USER WITH PASSWORD '$GREENSQL_DB_PWD';"; then
          echo "Failed to create user4 $GREENSQL_DB_USER"
          echo "Aborting.."
          exit 0
        fi
      fi
    ...
    if [ "$SKIP_USER" = "0" ]; then
      if [ "$EXISTS_USER" = "1" ]; then
        ...
        if ! $PSQL -h $POSTGRES_HOST -p $POSTGRES_PORT $POSTGRES_ADMIN_USER postgres -c "CREATE USER $GREENSQL_DB_USER WITH PASSWORD '$GREENSQL_DB_PWD';"; then
          echo "Failed to create user5 $GREENSQL_DB_USER"
          echo "Aborting.."
          exit 0
        fi
      fi
