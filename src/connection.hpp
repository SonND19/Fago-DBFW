...
public:
    ...
    bool first_response;
    void copy_sql(std::string &query);
    // bool AddPermission(std::string name, std::string user);
    // std::string db_perssion_name;
    // std::string db_perssion_use;
private:
    unsigned int calculateRisk(std::string & query, std::string &reason);
    std::string question(std::string &query, std::string &reason);
    std::string block_reason(std::string &query, std::string &reason);
