...
#endif
{
    ...
    if (Log_init(cfg->file_event, cfg->log_level) == false)
    {
        fprintf(stderr, "Failed to open log file %s\n",
                            cfg->file_event.c_str());
        return -1;
    }
    if (LogInit(cfg->file_alert, cfg->log_level) == false)
    {
        fprintf(stderr, "Failed to open log file %s\n",
                            cfg->file_alert.c_str());
        return -1;
    }
	...
	if (pgsql_patterns_init(conf_path) == false)
	{
        ...
        //de quan ly delay_time cua  event
        struct event tEvent;
        memset(&tEvent,0, sizeof(struct event));
        //*std::cout << "sizeof = " << sizeof(struct event) << "\n";
