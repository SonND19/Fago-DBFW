...
bool dbmap_close()
{
    ...
    while (dbs.size() != 0)
    {
        ...
        delete itr->second;
	dbs.erase(itr);
    std::cout << "dbmapclose1\n";
    }
    dbs.clear();
    std::cout << "dbmapclose2\n";
    return true;
}
...
{std::cout << "dbmap0\n";
    ...
    //"info_perm, block_q_perm,grant_perm, revoke,perm  "
    ...
    if (!db_exec(q_learning3)) 
    { 
        logevent(STORAGE,"DB config erorr: %s\n",db_error()); 
        std::cout << "config 1\n";
        return false; 
    }
    if (!db_exec(q_learning7)) 
    { 
        logevent(STORAGE,"DB config erorr: %s\n",db_error()); 
        std::cout << "config 2\n";
        return false; 
    ...
    /* read all database */
    if (!db_query(&db,q_db,256)) {//std::cout << "dbmap3\n";
        logevent(STORAGE,"DB config erorr: %s\n",db_error());
        std::cout << "DB config erorr8\n";
        db_cleanup(&db);
        return false;
    } 
    ...
    if (!db_query(&db,q_default_db,256)) {//std::cout << "dbmap0\n";
        logevent(STORAGE,"DB config erorr: %s\n",db_error());
        std::cout << "DB config erorr9\n";
        db_cleanup(&db);
        return false;
    } 
