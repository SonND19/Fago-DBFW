...
static int(*p_db_permiss)(const char *) = NULL;
static int(*p_db_backup)(const char *) = NULL;
// static int(*p_db_restore)(const char *) = NULL;
...
int db_init(const std::string& type)
{
  ...
  //std::cout << "p_dbrow= " << p_db_fetch_row << "\n";
  if ((dlerror()) != NULL)
  {
  ...
  p_db_permiss = (int (*)(const char *))dlsym(lib_handle, "db_permiss");
  if ((dlerror()) != NULL)
  {
    logevent(ERR, "failed to load function db_permiss: %s\n", dlerror());
    return 0;
  }
  p_db_backup = (int (*)(const char *))dlsym(lib_handle, "db_backup");
  if ((dlerror()) != NULL)
  {
    logevent(ERR, "failed to load function db_backup: %s\n", dlerror());
    return 0;
  }
  // p_db_restore = (int (*)(const char *))dlsym(lib_handle, "db_restore");
  // if ((dlerror()) != NULL)
  // {
  //   logevent(ERR, "failed to load function db_restore: %s\n", dlerror());
  //   return 0;
  // }
#endif
  return 1;
...
{
  if(p_db_permiss == NULL)
    return NULL;
  return (*p_db_permiss)(q);
}
// int db_restore(const char *path)
// {
//   if(p_db_restore == NULL)
//     return NULL;
//   return (*p_db_restore)(path);
// }
int db_backup(const char *path)
{
  if(p_db_backup == NULL)
    return NULL;
  return (*p_db_backup)(path);
}
#ifdef __cplusplus
}
#endif
