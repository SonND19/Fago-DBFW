...
//     return 1;
    ...
    int num = PQntuples(pgres_temp);
    if(num > 0){
        for(int i = 0; i < num; i++){
            char *ress = PQgetvalue(pgres_temp, i, 0);
            // char *ress_1 = PQgetvalue(pgres_temp, i, PQfnumber(pgres_temp, 'privilege_type'));
            if (strcmp(ress, "t") == 0){
                // PQclear(pgres_temp);
                // PQclear(pgres_temp);
                // pgres_temp = NULL;
...
int db_backup(const char *path)
{
...
#ifdef HAVE_PGSQL_CLIENT
    pgres_temp = PQexec(pgsqldbConn,path);
    if (!pgres_temp)
    {
        return 0;
    }
    if(PQresultStatus(pgres_temp) != PGRES_TUPLES_OK)//== PGRES_COMMAND_OK)
    {
        PQclear(pgres_temp);
        pgres_temp = NULL;
        return 1;
    ...
    int rows = PQntuples(pgres_temp);
    if (rows == 1) {
        char *res = PQgetvalue(pgres_temp, 0, 0);
        if(strcmp(res, "t") == 0){
                // PQclear(pgres_temp);
            return 1;
        }
...
//         return 0;
//     }
//     if(PQresultStatus(pgres_temp) == PGRES_COMMAND_OK)
//     {
//         PQclear(pgres_temp);
//         return 1;
//     }
