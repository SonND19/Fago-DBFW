...
void GreenSQLConfig::init()
{
    ...
    file_alert = "/var/log/alert.log";
    file_event = "/var/log/event.log";
}
...
bool GreenSQLConfig::parse_log_setting(std::string & key, std::string & value){
    ...
    {
        log_file = value;
    } else if (key == "filealert")
    {
        file_alert = value;
    } else if(key == "fileevent")
    {
        file_event = value;
    }
    return true;    
...
bool GreenSQLConfig::load_db(){
    ...
    if (db_load(sDbHost.c_str(),sDbUser.c_str(),sDbPass.c_str(),sDbName.c_str(),iDbPort) == 0)
    {
        ...
        std::cout << sDbHost << sDbUser << sDbPass << sDbName << iDbPort << "\n";
        return false;
    }
    return true;
}
