...
bool PgSQLConnection::parseRequest(std::string & request, bool & hasResponse)
{
    ...
    if (full_size == 8)
    {
        ...
        if (client_flags & PGSQL_CON_SSL)std::cout << "parseRequest2\n";
...
bool PgSQLConnection::parse_request(const unsigned char * data, size_t request_size,bool & hasResponse) {
	...
	if (first_request) 
	{
		if (request_size < 13) {
			...
			loghex(SQL_DEBUG, data, 12);std::cout << "request\n";
			return false;
		}
	...
	case PGSQL_QUERY:
		...
		{
			// Log(db_user.c_str(),db_name.c_str(), original_query.c_str(),BLOCKED);
			// bad query - block it
