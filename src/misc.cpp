...
// bool check_pwd(std::tring &pass)
// {
//     bool hasUpperCase = false;
//     bool hasNumber = false;
//     bool hasSpeciaChar = false;
//     for(int i = v[0]; i <v.size(); i++)
//     {
//         if(isupper(v))
//         {
//             hasUpperCase = true;
...
//         }
//     }
//     if (!hasNumber || !hasUpperCase || !hasSpeciaChar)
//         return false;
//     return true;
// }
...
std::string search_per(const std::string &query_sql)
{
    std::regex permiss("(select|delete|update|insert|drop|truncate|create)", std::regex_constants::icase);
    std::smatch command;
    if(std::regex_search(query_sql, command,permiss))
    {
        return command.str();
    }
    else
...
std::string search_table(const std::string &query_sql)
{
    std::regex permiss("(admin|info|alert|query|alert_group|proxy|db_perm)", std::regex_constants::icase);
    std::smatch command;
    if(std::regex_search(query_sql, command,permiss))
    {
        return command.str();
